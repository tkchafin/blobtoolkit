...
if (params.blastp) { ch_blastp = Channel.fromPath(params.blastp) } else { exit 1, 'Diamond BLASTp database must be specified!' }
if (params.blastx) { ch_blastx = Channel.fromPath(params.blastx) } else { exit 1, 'Diamond BLASTx database must be specified!' }
if (params.blastn) { ch_blastn = Channel.fromPath(params.blastn) } else { exit 1, 'BLASTn database not specified!' }
if (params.taxdump) { ch_taxdump = Channel.fromPath(params.taxdump) } else { exit 1, 'NCBI Taxonomy database not specified!' }
...
workflow BLOBTOOLKIT {
    ...
    BUSCO_DIAMOND (
        ...
        INPUT_CHECK.out.ch_busco_db,
        INPUT_CHECK.out.ch_blastp,
